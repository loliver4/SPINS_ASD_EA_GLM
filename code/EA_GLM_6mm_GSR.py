...
file_handler = logging.FileHandler(
    "/projects/loliver/SPINS_ASD_EA_GLM/data/EA_GLM_6mm_GSR.log"
)
...
def collect_preprocessed_files(sub_indir, subject_id):
    ...
    in_files["ortvec_file"] = os.path.join(
        sub_indir,
        next(f for f in sub_files if f.endswith("ea_confounds_GSR_glm.1D")),
    )
    in_files["AM2"] = [
        (
            1,
...
def main():
    ...
    for f in os.listdir(in_path):
        if "1_desc-preproc_Atlas_s6.dtseries.nii" in f:
    ...
    #    "sub-"
    #    + sub_id
    #    + "_ses-01_task-emp_run-1_desc-preproc_Atlas_s6.dtseries.nii",
    # )
